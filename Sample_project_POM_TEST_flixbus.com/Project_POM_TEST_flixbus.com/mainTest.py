...
class FlixbusTest(unittest.TestCase):
    ...
    def test_01_check_page_loading(self):
        ...
        driver = self.driver
        expected_title = Dictionaries.test_1.get("title")
    ...
    def test_03_sample_search_test(self):
        ...
        Uses test_3 dictionary from dictionaries module"""
        driver = self.driver
        search = SearchPage(driver)

        # Performing user steps:
        search.select_two_ways_flight()
        search.insert_departure(Dictionaries.test_3.get("departure"))
        search.insert_arrival(Dictionaries.test_3.get("arrival"))
        search.replace_destinations()
        search.insert_dates(Dictionaries.test_3.get("date"))
        ...
        with self.subTest("Checking if main message is equal to expected"):
            self.assertTrue(result.assertion_by_text(Dictionaries.test_3.get("assert_text")))
        with self.subTest("Checking if departure place is equal to expected"):
            self.assertTrue(result.departure_assertion(Dictionaries.test_3.get("assert_departure")))
        with self.subTest("Checking if arrival place is equal to expected"):
            self.assertTrue(result.arrival_assertion(Dictionaries.test_3.get("assert_arrival")))
        with self.subTest("Checking if departure date is equal to expected"):
            self.assertTrue(result.date1_assertion(Dictionaries.test_3.get("assert_date1")))
        with self.subTest("Checking if arrival date is equal to expected"):
            self.assertTrue(result.date2_assertion(Dictionaries.test_3.get("assert_date2")))
        with self.subTest("Checking if adults, children, bikes amount is equal to expected"):
            self.assertTrue(result.passengers_assertion(Dictionaries.test_3.get("assert_passengers")))
    ...
    def test_04_sample_search_test_with_fail(self):
        ...
        Uses test_4 dictionary from dictionaries module"""
        driver = self.driver
        driver.get("https://www.flixbus.com/")
        search = SearchPage(driver)
        ...
        # Performing user steps:
        search.select_two_ways_flight()
        search.insert_departure(Dictionaries.test_4.get("departure"))
        search.insert_arrival(Dictionaries.test_4.get("arrival"))
        search.insert_dates(Dictionaries.test_4.get("date"))
        ...
        with self.subTest("Checking if main result page message is equal to expected"):
            ...
            self.assertTrue(result.assertion_by_text(Dictionaries.test_4.get("assert_text")))
        with self.subTest("Checking if departure place is equal to expected"):
            self.assertTrue(result.departure_assertion(Dictionaries.test_4.get("assert_departure")))
        with self.subTest("Checking if arrival place is equal to expected"):
            self.assertTrue(result.arrival_assertion(Dictionaries.test_4.get("assert_arrival")))
        with self.subTest("Checking if departure date is equal to expected"):
            self.assertTrue(result.date1_assertion(Dictionaries.test_4.get("assert_date1")))
        with self.subTest("Checking if arrival date is equal to expected"):
            self.assertTrue(result.date2_assertion(Dictionaries.test_4.get("assert_date2")))
        with self.subTest("Checking if adults, children, bikes amount is equal to expected"):
            self.assertTrue(result.passengers_assertion(Dictionaries.test_4.get("assert_passengers")))
